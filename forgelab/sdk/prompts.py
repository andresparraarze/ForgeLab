...
_REFERENCE_HINTS: dict[str, str] = {
    ...
    "mechanical": (
        ...
        "(see examples/mechanical/rounded_rect_plate.forge.json). A pocket "
        "cuts along the NEGATIVE normal of its sketch plane, so a pocket "
        "sketched on XY over a pad that rises from z=0 cuts downward into "
        'empty space and removes nothing: set "reversed": true (and '
        '"through_all": true for a hole straight through, see '
        "examples/mechanical/box-with-hole.forge.json). The "
        ...
        "empty. Worse, a feature can build a perfectly valid solid without "
        "doing what you asked: a pocket cutting the wrong way leaves the part "
        "whole and hole-less. After building a mechanical document, call "
        "verify_geometry on it: it recomputes the part in the real FreeCAD "
        "kernel and names the specific node that produced no solid, or that "
        "produced the same solid it started with. Then call preview_render to see "
